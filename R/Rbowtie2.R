...
bowtie2_build <- function(references, outdir, ..., prefix="index", force=FALSE, strict=TRUE)
{
    ...
    args <- sprintf("%s %s %s/%s", .createFlags(list(...)), paste(references, collapse=","),
                    outdir, prefix)
    return(invisible(.bowtieBin("bowtie2-build", args)))
}


## The main wrapper around bowtie
bowtie2 <- function(sequences, index, ..., type=c("single", "paired", "crossbow"), outfile,
                   force=FALSE, strict=TRUE)
{
    ...
    return(invisible(.bowtieBin("bowtie2-align", args)))
}

## Little helpers that return a description of the intended usage for bowtie and bowtie-build
bowtie2_build_usage <- function()
    print(bowtie2_build("dummy", "dummy", force=TRUE, usage=TRUE, strict=FALSE))

bowtie2_usage <- function()
    print(bowtie2("dummy", "dummy", force=TRUE, usage=TRUE, strict=FALSE))
...
.bowtieBin <- function(bin=c("bowtie2-align", "bowtie2-build"), args="")
{
    if(is.null(args) || args=="")
        stop("The bowtie binaries need to be called with additional arguments")
    args <- gsub("^ *| *$", "", args)
    bin <- match.arg(bin)
    call <- paste(file.path(system.file(package="Rbowtie2"), bin), args)
    #return(call)
...
.execute <- function(callstr){
  call <- file.path(system.file(package="Rbowtie2"), callstr)
